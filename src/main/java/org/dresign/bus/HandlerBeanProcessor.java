...
public class HandlerBeanProcessor<C extends Annotation> {
	...
	public HandlerBeanProcessor(Bus bus,Object handlerCandidate, Class<C> annotation) {
		this.bus=bus;
		this.handlerCandidate=handlerCandidate;
		this.annotationClazz=annotation;
	}
	
	private List<Method> findAnnotatedMethods(Class<?> clazz, Class<C> annotationClazz) {
	...
	public void analyseCandidateForSubscribe() {
		...
		for(Method m : methods ){
			if(m.getParameterTypes().length!=1){
				throw new RuntimeException("Your Handler should only have one parameter");
			}
			if(m.getParameterTypes()[0].getAnnotations().length!=1){
				if(!m.getParameterTypes()[0].getAnnotations()[0].annotationType().isAssignableFrom(DomainEvent.class)){
					throw new RuntimeException("Your Handler should only have one parameter of Type :"+DomainEvent.class.getSimpleName());
				}
			}
